...
st.markdown(
    ...
    <div class="cibia-container">
        ...
        <p class="cibia-description"><strong>La IA de CIB</strong></p>
    </div>
    """,
...
sidebar_image_path = os.path.join(script_dir, '..', 'img', 'cibia_left.png')
...
if st.session_state.start_chat:
    ...
    if prompt := st.chat_input("What would you like to say"):
        ...
        # Mostrar el nuevo mensaje del usuario con la imagen del usuario
        st.markdown(f"""
            <div class="chat-message">
                <img src="data:image/png;base64,{user_image}" class="chat-image" alt="Usuario">
                <div class="chat-text user-text">{prompt}</div>
            </div>
        """, unsafe_allow_html=True)
        ...
        for message in assistant_messages_for_run:
            # Reemplaza los enlaces en el formato de referencia con enlaces HTML
            #formatted_content = message.content[0].text.value
            formatted_message = message.content[0].text.value.replace("【", "[").replace("†source】", "](source)")

            st.session_state.messages.append({"role": "assistant", "content": formatted_message})
            st.markdown(f"""
                <div class="chat-message">
                    <img src="data:image/png;base64,{assistant_image}" class="chat-image" alt="Asistente">
                    <div class="chat-text assistant-text">{formatted_message}</div>
                </div>
            """, unsafe_allow_html=True)
else:
